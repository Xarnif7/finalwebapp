...
-- Simplified version that doesn't require automation_templates to exist yet

-- Create scheduled_jobs table without foreign key to automation_templates initially
CREATE TABLE IF NOT EXISTS scheduled_jobs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id UUID NOT NULL REFERENCES businesses(id) ON DELETE CASCADE,
  template_id UUID, -- Will add foreign key constraint later
...
-- Add foreign key constraint to automation_templates if the table exists
DO $$
BEGIN
