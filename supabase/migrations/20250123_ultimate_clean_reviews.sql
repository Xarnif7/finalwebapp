-- ULTIMATE CLEAN RESET - Handles all existing objects
-- This will work even if some objects already exist

-- Drop ALL related tables and constraints in correct order
...
-- Drop any existing indexes (in case they exist)
DROP INDEX IF EXISTS idx_review_sources_business_id CASCADE;
DROP INDEX IF EXISTS idx_review_sources_platform CASCADE;
DROP INDEX IF EXISTS idx_review_sources_is_active CASCADE;
DROP INDEX IF EXISTS idx_reviews_business_id CASCADE;
DROP INDEX IF EXISTS idx_reviews_platform CASCADE;
DROP INDEX IF EXISTS idx_reviews_status CASCADE;
DROP INDEX IF EXISTS idx_reviews_assigned_to CASCADE;
DROP INDEX IF EXISTS idx_reviews_due_at CASCADE;
DROP INDEX IF EXISTS idx_reviews_rating CASCADE;
DROP INDEX IF EXISTS idx_reviews_sentiment CASCADE;
DROP INDEX IF EXISTS idx_reviews_review_created_at CASCADE;
DROP INDEX IF EXISTS idx_reviews_tags CASCADE;
DROP INDEX IF EXISTS idx_reviews_topics CASCADE;
DROP INDEX IF EXISTS idx_templates_business_id CASCADE;
DROP INDEX IF EXISTS idx_templates_platform CASCADE;
DROP INDEX IF EXISTS idx_audit_log_review_id CASCADE;
DROP INDEX IF EXISTS idx_audit_log_created_at CASCADE;
...
DROP POLICY IF EXISTS "Users can view their own business reviews" ON public.reviews;
DROP POLICY IF EXISTS "Users can insert reviews for their business" ON public.reviews;
DROP POLICY IF EXISTS "Users can update their own business reviews" ON public.reviews;
DROP POLICY IF EXISTS "Users can delete their own business reviews" ON public.reviews;
DROP POLICY IF EXISTS "Users can manage their own business templates" ON public.review_response_templates;
DROP POLICY IF EXISTS "Users can view audit logs for their business reviews" ON public.review_audit_log;
DROP POLICY IF EXISTS "Users can insert audit logs for their business reviews" ON public.review_audit_log;

-- Now create everything fresh with correct types

-- 1. Create review_sources table with UUID business_id
...
-- 2. Create reviews table with UUID business_id
...
-- 3. Create response templates table
...
CREATE INDEX IF NOT EXISTS idx_reviews_due_at ON public.reviews(due_at);
CREATE INDEX IF NOT EXISTS idx_reviews_rating ON public.reviews(rating);
CREATE INDEX IF NOT EXISTS idx_reviews_sentiment ON public.reviews(sentiment);
CREATE INDEX IF NOT EXISTS idx_reviews_review_created_at ON public.reviews(review_created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_templates_business_id ON public.review_response_templates(business_id);
CREATE INDEX IF NOT EXISTS idx_templates_platform ON public.review_response_templates(platform);

CREATE INDEX IF NOT EXISTS idx_audit_log_review_id ON public.review_audit_log(review_id);
CREATE INDEX IF NOT EXISTS idx_audit_log_created_at ON public.review_audit_log(created_at DESC);
