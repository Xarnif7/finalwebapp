-- Enhanced Reviews System - Final Fixed Schema
-- Drop existing reviews table and recreate with full functionality

DROP TABLE IF EXISTS public.reviews CASCADE;

-- Enhanced reviews table with all required fields
CREATE TABLE public.reviews (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    business_id UUID NOT NULL, -- Changed to UUID to match businesses.id
...
-- Audit log table for tracking all review actions
...
-- Review response templates table
...
-- Review sources table for connected platforms
...
DROP INDEX IF EXISTS idx_reviews_platform;
DROP INDEX IF EXISTS idx_reviews_status;
DROP INDEX IF EXISTS idx_reviews_assigned_to;
DROP INDEX IF EXISTS idx_reviews_due_at;
DROP INDEX IF EXISTS idx_reviews_rating;
DROP INDEX IF EXISTS idx_reviews_sentiment;
DROP INDEX IF EXISTS idx_reviews_review_created_at;
DROP INDEX IF EXISTS idx_reviews_tags;
DROP INDEX IF EXISTS idx_reviews_topics;
DROP INDEX IF EXISTS idx_audit_log_review_id;
...
-- RLS Policies for reviews (fixed type matching)
...
-- RLS Policies for audit log (fixed type matching)
...
-- RLS Policies for templates (fixed type matching)
...
DROP TRIGGER IF EXISTS update_templates_updated_at ON public.review_response_templates;
DROP TRIGGER IF EXISTS audit_reviews_changes ON public.reviews;
DROP TRIGGER IF EXISTS auto_classify_sentiment ON public.reviews;
DROP TRIGGER IF EXISTS set_review_sla ON public.reviews;
