-- Create tables for feedback collection system (FIXED VERSION)
...
-- 6. Create RLS policies (using business_id for multi-tenancy)
CREATE POLICY "Users can manage review requests for their business" ON public.review_requests
    FOR ALL USING (
        business_id IN (
...
CREATE POLICY "Users can manage templates for their business" ON public.templates
    FOR ALL USING (
        business_id IN (
            SELECT business_id FROM public.profiles 
            WHERE user_id = auth.uid()
