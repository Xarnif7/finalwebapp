...
app.post('/api/billing/portal', async (req, res) => {
  try {
    ...
    const returnUrl = `${process.env.APP_BASE_URL || process.env.VITE_SITE_URL || 'https://myblipp.com'}/settings?tab=billing`;
...
// Test Send API - Based on send-followup-email but for template testing
app.post('/api/send-test', async (req, res) => {
  try {
    ...
    const { to, message, businessId, customerId, templateId } = req.body || {};
    ...
    let businessEmail = 'noreply@myblipp.com';
    ...
    if (businessId) {
      const { data: business } = await supabase
        ...
        .single();
      if (business?.name) businessName = business.name;
      if (business?.email) businessEmail = business.email;
    }

    // Personalize message
    const personalizedMessage = message
      .replace(/\{\{customer\.name\}\}/g, 'Test Customer')
      .replace(/\{\{business\.name\}\}/g, businessName)
      .replace(/\{\{review_link\}\}/g, `https://www.google.com/search?q=${encodeURIComponent(businessName)}`);

    // Send email via Resend (same as send-followup-email)
    const emailResponse = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${process.env.RESEND_API_KEY}`,
        ...
        html: `
          <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
            ...
            <div style="text-align: center; padding: 20px; background: #f8f9fa; color: #666; font-size: 12px;">
              This is a test email sent from Blipp
            </div>
          </div>
        `,
...
app.post('/api/profile/ensure-setup', async (req, res) => {
  try {
    ...
    if (existingProfile) {
      // User already has a profile, just return the business_id
...
app.get('/api/qbo-oauth-callback', async (req, res) => {
  try {
    ...
    if (verifyError || !savedIntegration) {
      console.error('[QBO] Failed to verify saved connection:', verifyError);
      return res.status(500).send(`
        <html>
          <body>
            <h1>QuickBooks Connection Error</h1>
            <p>Connection was not saved properly. Please try again.</p>
            <script>
              window.close();
            </script>
          </body>
    ...
    console.log('[QBO] Connection verified in database:', savedIntegration.id);
...
app.post('/api/test-sms-direct', async (req, res) => {
  try {
    ...
    // Use the known business configuration
    ...
    console.log(`[TEST_SMS] Sending SMS from ${fromNumber} to ${e164Phone}`);
    ...
    if (!surgeResponse.ok) {
      console.error('[TEST_SMS] Surge API error:', surgeResult);
      return res.status(500).json({
        error: 'Failed to send SMS',
        details: surgeResult.error || 'Surge API error'
      });
    ...
    console.log('[TEST_SMS] Message sent successfully:', surgeResult.message_id || surgeResult.id);
